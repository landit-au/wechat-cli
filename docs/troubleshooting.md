...
Symptom → cause → fix for the CLI itself. Problems in whatever consumes its
output belong to that consumer's own docs, not here.
...
| `init` reports "提取到 0 个密钥" despite a successful scan | Cosmetic misreport bug in huohuoer (indexes on a `"salt"` key that doesn't exist in the C binary's JSON schema) | Harmless — fixed upstream in huohuoer PR #4 |
...
| Raw voice/image XML with live `aeskey`/`voiceurl` in `history` output | Non-text `msg_type` content passed through unparsed | Consumers must detect non-text types and emit placeholders — never forward/store the raw XML |
